...
supplied for that attempt (no last-good fallback), staleness is computed
from those ranks against the report wall clock, sequence IDs are shared,
positive, and monotonically increasing across both build paths, and an
unreachable report carries no ranks plus a normalized diagnostic.
"""
...
class TestBuild:
    ...
    def test_empty_ranks_are_rejected(self):
        with pytest.raises(ValueError):
            make_builder().build(
                (), make_identity(), report_time_unix_ms=20_100
            )
...
class TestBuildUnreachable:
    ...
    def test_exception_error_uses_its_message(self):
        report = make_builder().build_unreachable(
            make_identity(), report_time_unix_ms=30_000, error=RuntimeError("read failed")
        )
